...
The [*Lognormal distribution*](https://en.wikipedia.org/wiki/Log-normal_distribution)
with mean `μ` [`Real`], standard deviation `σ` > 0 [`Real`] and with optional weight `ω` has a probability density
function

```math
f(x; μ, σ, ω) = \begin{cases}
                    ω / ( xσ√(2π) ) e^( -(ln(x) - μ)² / 2σ²)        & x ≥ 0, \\
                    0                                               & x < 0.
                \end{cases}
```
"""
...
# constructors
Lognormal() = Lognormal(0.0, 1.0, 1.0)
Lognormal(μ::Number) = Lognormal(μ, 1.0, 1.0)
Lognormal(μ::Number, σ::Number) = Lognormal(μ, σ, 1.0)
Lognormal(μ::Number, σ::Number, ω::Real) = Lognormal(μ, σ, ω)
...
minimum(dst::Lognormal) = zero(dst.θ)
